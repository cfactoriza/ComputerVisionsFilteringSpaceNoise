...
def bilateral_filtering(
    ...
    for i in range(pad, sizeX + pad):
        for j in range(pad, sizeY + pad):
            window = img_pad[i - pad:i + pad + 1, j - pad:j + pad + 1]
            spatial_weights = np.exp(
                -((np.arange(-pad, pad + 1)[:, None]) ** 2 + (np.arange(-pad, pad + 1)[None, :]) ** 2) / (
                            2 * spatial_variance))
            intensity_weights = np.exp(-((window - img_pad[i, j]) ** 2) / (2 * intensity_variance))
            weights = spatial_weights * intensity_weights
            weights = weights / (np.sum(weights) + 1e-16)
            img_filtered[i - pad, j - pad] = np.sum(weights * window)
    # Your code is here.
    # ********************************
